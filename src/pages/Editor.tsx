...
const SmartFrameStation = ({ imgSrc, mockupDef, initialScale, initialX, initialY, onSave, onClose }: {
  ...
  // Lock body scroll while the station is open
  useEffect(() => {
  ...
  // Device screen hole dimensions (aspect ratio MUST match canvas dropzone)
  ...
  useEffect(() => {
    ...
    const handleMove = (e: MouseEvent) => {
      ...
      // Because we apply: translate(%) scale(scale), the translation is scaled.
      // Convert pixel delta back to % in the unscaled image box.
      setNormX((v) => v + (dx / (HOLE_W * Math.max(scale, 0.0001))) * 100);
      setNormY((v) => v + (dy / (HOLE_H * Math.max(scale, 0.0001))) * 100);
    };
  ...
  return (
    <motion.div
      ...
      onMouseDown={(e) => {
        // click outside panel closes only if click happens on the root backdrop
        if (e.target === e.currentTarget) onClose();
      }}
    >
      {/* ─── LEFT: Framing Canvas (75%) ─── */}
      <div
        ...
        onWheel={handleWheel}
      >
        {/* Crop box (same aspect ratio as the real device screen hole) */}
        <div
          className="absolute"
          style={{
            width: HOLE_W,
        ...
        {/* Smart mask overlay (fixed, not moving with the image) */}
        ...
        {/* Dynamic Island indicator for iPhone */}
      ...
      <div className="flex-[1] flex flex-col" style={{ background: "hsl(0 0% 0%)", borderLeft: "1px solid hsl(0 0% 100% / 0.06)" }}>
        <div className="p-6 pb-4" style={{ borderBottom: "1px solid hsl(0 0% 100% / 0.06)" }}>
          ...
          <p className="text-xs" style={{ color: "hsl(0 0% 100% / 0.35)" }}>
            Re-encuadra sin perder el estado actual
          </p>
        </div>
  ...
  const [scale, setScale] = useState(initialScale || 1);
  const [panX, setPanX] = useState(initialX || 0);
  const [panY, setPanY] = useState(initialY || 0);
  const [dragging, setDragging] = useState(false);
  const lastPos = useRef({ x: 0, y: 0 });
  ...
  // The "hole" display size — fit nicely in the canvas
  const HOLE_H = 420;
  const HOLE_W = Math.round(HOLE_H * aspectRatio);
  const holeRadius = mockupDef.id === "iphone15" ? 36 : mockupDef.id === "tablet" ? 16 : 12;

  // Unique mask id to avoid conflicts
  const maskId = useMemo(() => `sfs-mask-${Math.random().toString(36).slice(2, 6)}`, []);
    ...
    >
      <div className="flex h-full w-full">
        ...
        <div
          ...
          onWheel={handleWheel}
        >
          {/* THE IMAGE — moves freely, full size, behind the mask */}
          <img
            src={imgSrc}
            alt="Original"
            draggable={false}
            ...
            className="absolute pointer-events-none"
            style={{
              width: imgNatural.w * 0.6,
              height: imgNatural.h * 0.6,
              left: "50%",
              top: "50%",
          ...
          {/* SMART MASK — stays FIXED in center, creates the device hole */}
          <svg
          ...
          >
            <defs>
              <mask id={maskId}>
                {/* White = visible overlay (dark tint) */}
                <rect width="100%" height="100%" fill="white" />
                {/* Black = transparent hole (shows the image clearly) */}
                <rect
                  x="50%"
                  y="50%"
                  width={HOLE_W}
            ...
            {/* The dark overlay with the hole punched out */}
            <rect
              width="100%"
              height="100%"
              fill="rgba(0,0,0,0.7)"
          ...
          {/* Device frame outline (subtle, fixed in center) */}
          <div
            className="absolute pointer-events-none"
            style={{
              width: HOLE_W,
          ...
          {mockupDef.notch && mockupDef.id === "iphone15" && (
            <div
              ...
              style={{
                ...
                left: "50%",
                top: "50%",
                transform: `translate(-50%, calc(-50% - ${HOLE_H / 2 - 24}px))`,
                borderRadius: 14,
          ...
          <div className="absolute pointer-events-none" style={{ left: "50%", top: "50%", transform: "translate(-50%, -50%)" }}>
            <div style={{ position: "absolute", width: 1, height: HOLE_H + 40, left: 0, top: -(HOLE_H + 40) / 2, background: "hsl(var(--primary) / 0.12)" }} />
            <div style={{ position: "absolute", height: 1, width: HOLE_W + 40, top: 0, left: -(HOLE_W + 40) / 2, background: "hsl(var(--primary) / 0.12)" }} />
          </div>
          ...
          >
            ...
            <span className="w-px h-3" style={{ background: "rgba(255,255,255,0.15)" }} />
            <span className="text-[10px] text-gray-500">Scroll zoom · Drag to pan</span>
          </div>
        </div>

        {/* ─── RIGHT: Control Panel (25%) ─── */}
        <div className="flex-[1] flex flex-col border-l" style={{ background: "#000000", borderColor: "rgba(255,255,255,0.06)" }}>
          {/* Header */}
          <div className="p-6 pb-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            <div className="flex items-center justify-between mb-1">
              <h2 className="text-base font-bold text-white tracking-tight">Encuadre</h2>
              <button
                onClick={onClose}
                className="w-7 h-7 rounded-md flex items-center justify-center transition-colors"
            ...
            <p className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>{mockupDef.name}</p>
          </div>
          ...
          <div className="flex-1 p-6 space-y-7 overflow-y-auto">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <label className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.5)" }}>
                  Zoom
                </label>
                <span className="text-xs font-mono font-semibold" style={{ color: "hsl(var(--primary))" }}>
                  {Math.round(scale * 100)}%
          ...
          {/* Action Buttons */}
          <div className="p-6 space-y-3" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            <button
              onClick={onClose}
              className="w-full py-3 rounded-lg text-sm font-semibold transition-all"
              style={{
              ...
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = "hsl(var(--primary) / 0.5)"; e.currentTarget.style.background = "hsl(var(--primary) / 0.05)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.borderColor = "hsl(var(--primary) / 0.25)"; e.currentTarget.style.background = "transparent"; }}
            >
              Cancelar
            </button>
            <button
      ...
      <style>{`
        ...
        .sfs-slider::-webkit-slider-thumb:hover {
